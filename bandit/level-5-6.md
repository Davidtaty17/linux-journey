...
=> Once taped you will see several directories. In this time we will use the command < find > + < ./ > + < -size 1033c > to know in which directory our file is.
=> W will notice that our file was an hidden file because there is a dot < . > at the beginning of the filename.
=> Then, we will go these directory and open the file with < cat ./[file's name] >
=> Log out with < CTRL + D > or type < exit > on the terminal 


# Observations #
- < find -size > helps us to find a size of a file. We added *c* to determine the type's siez of the file. 
