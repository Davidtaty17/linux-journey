...
Loaction:<inhere directory> / Stored in a hidden file
Commands for this level: ls, cd, cat, file, find


How to slove this level ?
=> Log into the game using ssh with < bandit3 > as username and the password foundd in the previous level.
...
=> Once the hidden file locate, we will use the command < cat ./...Hiding-From-You > for printing the content on the screen.
=> Log out with < CTRL + D > or type < exit > on the terminal 


Observations
- In this level, you should make sure that you are written corretly the directory name and the de hidden file as you will see on the terminal.
